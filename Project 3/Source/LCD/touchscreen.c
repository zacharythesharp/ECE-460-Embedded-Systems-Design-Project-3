...
uint32_t LCD_TS_Read(PT_T * position) {
	...
	message.FlagNum = 0x00000001U;
	message.TID = t_Read_TS;
	...
	if (b>0) {
		...
		message.ChannelNum = LCD_TS_YU_CHANNEL;
		osMessageQueuePut(ADC_Server, &message, NULL, osWaitForever);
		osThreadFlagsWait(0x00000001U, osFlagsWaitAny, osWaitForever);
		
		//yu = ADC0->R[0];
		//yu = *message.ResultPtr;
		x = *pointer_to_result;
	  ...
	  //while (!(ADC0->SC1[0] & ADC_SC1_COCO_MASK));
		message.ChannelNum = LCD_TS_XL_CHANNEL;
		osMessageQueuePut(ADC_Server, &message, NULL, osWaitForever);
		osThreadFlagsWait(0x00000001U, osFlagsWaitAny, osWaitForever);
		
		//xl = ADC0->R[0];
		//xl = *message.ResultPtr;
		y = *pointer_to_result;
